...
YARA is a tool built to identify and classify malware by searching for unique patterns, the digital fingerprints left behind by attackers. YARA scans code, files, and memory for subtle traces that reveal a threat’s identity.
## Why YARA Matters and When to Use It

YARA allows you to define your own rules, providing your own view of what constitutes "malicious" behavior.
...
In this example, you can see that the meta section contains fields like author, description, and date. These fields are not required, but they are highly recommended. They help you and other defenders understand what the rule is about, who created it, and when it was written.

Next, we see the strings section. This is where you define the actual clues, text, byte sequences, or regular expressions that YARA should search for.

Finally, the condition section tells YARA when to flag a match, based on the logic you define, in our case, any of the strings.

---

**Strings**
They represent the signatures of malicious activity in fragments of text, bytes, or patterns
In YARA, there are three main types of strings, each with its own purpose.

**Text strings  
They represent words or short text fragments that might appear in a file, script, or memory. By default, YARA treats text strings as ASCII and case-sensitive <span style="color:rgb(255, 0, 0)">The example below shows a simple rule that searches for the word <b>Christmas</b> inside a file:</span>
...
Sometimes, attackers try to hide their code by changing how text looks inside a file - using encoding, case tricks, or even encryption. YARA helps defenders counter these obfuscation methods with a few powerful modifiers that extend the capabilities of text strings:
...
**Hexadecimal strings  
**Sometimes, code doesn't leave readable words behind; instead, it hides in raw bytes deep inside executables or memory. That's when hexadecimal strings come to the rescue. Hex strings allow YARA to search for specific byte patterns, written in hexadecimal notation. This is useful when defenders need to detect malware fragments like file headers, shellcode, or binary signatures that can't be represented as plain text.
...
**Sometimes, code mutates, small changes in file names, URLs, or commands make it harder to detect using plain text or hex strings. That's where regular expressions come in. Regex allows defenders to write flexible search patterns that can match multiple variations of the same malicious string.  
...
used a trojan known as IcedID to steal credentials from systems.
analysts discovered that the malicious files spread across shared a common signature, the same MZ header found in executable malware
These samples were small, lightweight loaders designed to infiltrate systems and later summon more dangerous payloads. Let's write our YARA rule.
...
search for the keyword `TBFC:` followed by an ASCII alphanumeric keyword across the `/home/ubuntu/Downloads/easter` directory to extract the message sent

- The message format looks like:
    

`TBFC:<ASCII alphanumeric keyword>`
...
`yara -s tbfc.yar /home/ubuntu/Downloads/easter -r`
## The regex

`/TBFC:[A-Za-z0-9]+/`

## How to read this regex (left → right)

Think of a regex as a **pattern the computer tries to find in text or binary data**.
...
YARA uses `/ /` to indicate regex patterns.

### 2️⃣ `TBFC:`

This part is **literal text**.

It means:

- Match the exact characters `T B F C :`
...
So this matches **one alphanumeric ASCII character**, such as:
...
> “The previous thing must appear **one or more times**”
...
> “Match **at least one** alphanumeric character, and keep matching until it stops.”

--- 

## 1️⃣ `[A-Za-z0-9]` — what does this REALLY mean?

### Think of `[ ]` as a **choice box**

When a regex sees **square brackets `[ ]`**, it means:

> “Match **ONE single character** from inside this box”
...
|Text|Does `[A-Za-z0-9]` match it?|Why|
|---|---|---|
|`A`|✅ yes|single letter|
|`7`|✅ yes|single digit|
|`z`|✅ yes|single letter|
|`@`|❌ no|not a letter or number|
|`ab`|❌ no|that’s **two** characters|
## 2️⃣ `+` — what does this REALLY mean?

The `+` symbol **does NOT stand alone**.  
It always applies to **whatever comes just before it**.

---

### What does `+` mean?

> “Repeat the previous thing **one or more times**”
...
## 3️⃣ Now combine them (THIS IS THE KEY PART)

`[A-Za-z0-9]+`

Read it step by step:

1. `[A-Za-z0-9]` → match **one letter or number**
    
2. `+` → repeat that **one or more times**
    

So together it means:

> “Match **a sequence** of letters and/or numbers”
...
|Character|Allowed by `[A-Za-z0-9]`?|
|---|---|
|H|✅|
|o|✅|
|p|✅|
...
Since **all characters match**, and `+` allows repetition → ✅ MATCH
...
## 4️⃣ Final: Put it back into the full regex

`/TBFC:[A-Za-z0-9]+/`

Now read it in **human language**:

> “Find the exact text `TBFC:` and then match **one or more letters or numbers** after it.”
