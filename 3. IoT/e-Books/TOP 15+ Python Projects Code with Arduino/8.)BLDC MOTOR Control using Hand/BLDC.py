...
# Pin number for BLDC motor control (assuming it's connected to pin 9)
pin9 = board.get_pin('d:9:p')  # 'd:9:p' means digital pin 9, PWM mode
...
# Function to control the BLDC motor speed
def control_motor_speed(pwm_value):
    pin9.write(pwm_value)  # PWM value between 0 (off) and 1 (full speed)
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                pwm_value = np.interp(distance, [10, 200], [0, 1])  # PWM value between 0 and 1
                print(f"Distance: {distance}, PWM Value: {pwm_value}")
                control_motor_speed(pwm_value)
